...
for i in range(len(people) - 1):
    ...
    for j in range(len(people)):
        ...
        if (((people[i]['gender'] is not people[j]['gender']) and (
                    len(set(people[i]['interests']) & set(people[j]['interests'])) > 0) and (
                    'matched' not in people[i])) and ('matched' not in people[j])):
            # print('{0} and {1} - {2}'.format(people[i]['name'], people[j]['name'], len(set(people[i]['interests']) & set(people[j]['interests']))))
